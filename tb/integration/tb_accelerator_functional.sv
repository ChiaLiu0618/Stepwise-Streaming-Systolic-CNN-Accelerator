...
initial begin
    ...
    @(negedge clk);
    instruction_valid = 1'b1;
    instruction = pack_instruction(memory_op(5'b0, 1'b0, 1'b0, 1'b0, 1'b0, 1'b0, 1'b1, 1'b1), '0);  // change mode to FC, don't load weight, don't load bias and scale
    @(negedge clk);
    instruction_valid = 1'b0;
    instruction = '0;
...
task input_conv1_param_task; begin
    ...
    weight_idx = 0;
    instruction_valid = 1'b1;
    instruction = pack_instruction(memory_op(weight_idx, 1'b1, 1'b0, 1'b0, 1'b0, 1'b0, 1'b0, 1'b0), '0);  // don't change mode, load weight, don't load bias and scale
    ...
    for(m=0; m<8; m=m+1) begin      // 32 sets (layers) of weights, but only first 8 sets have value
        ...
        instruction = pack_instruction(memory_op(weight_idx, 1'b1, 1'b0, 1'b0, 1'b0, 1'b0, 1'b0, 1'b0), '0);  // don't change mode, load weight, don't load param
    end
    ...
    @(negedge clk);
    instruction_valid = 1'b1;
    instruction = pack_instruction(memory_op(5'b0, 1'b0, 1'b1, 1'b1, 1'b0, 1'b0, 1'b0, 1'b0), '0);  // don't change mode, don't load weight, load bias and scale
...
task input_conv1_task; begin
    for(m=0; m<8; m=m+1) begin
        ...
        if(m==7) instruction = pack_instruction('0, compute_op(activation_channel, weight_idx, 1'b1, 1'b1, 1'b1, 1'b1, 1'b0));  // Compute, Store, ReLU, Pool, Don't load Activation
        else instruction = pack_instruction('0, compute_op(activation_channel, weight_idx, 1'b1, 1'b0, 1'b0, 1'b0, 1'b0));  // Compute, Don't store, No ReLU, No Pool, Don't load Activation
    end
...
task input_conv2_task; begin
    ...
    @(negedge clk);
    instruction_valid = 1'b1;
    instruction = pack_instruction(memory_op(5'b0, 1'b0, 1'b0, 1'b0, 1'b1, 1'b0, 1'b0, 1'b0), compute_op(3'b0, 5'b0, 1'b0, 1'b0, 1'b0, 1'b0, 1'b0));  // Dont't compute, Don't store, No ReLU, No Pool, Load Activation
    for(j=0; j<4; j=j+1) begin      // 4 columns
        for(m=0; m<8; m=m+1) begin      // 8 sets (layers) of Activations
            for(i=0; i<4; i=i+1) begin
                SRAM_activation[i][j][m] = activation[i][j][m];
    ...
    for(m=0; m<7; m=m+1) begin
        ...
        activation_channel = m;
        weight_idx = m;
        instruction = pack_instruction('0, compute_op(activation_channel, weight_idx, 1'b1, 1'b0, 1'b0, 1'b0, 1'b0));  // Compute, Don't store, No ReLU, No Pool, Don't load Activation
    end
    ...
    weight_idx = 5'd7;
    instruction = pack_instruction(memory_op(5'b0, 1'b0, 1'b0, 1'b0, 1'b1, 1'b0, 1'b0, 1'b0), compute_op(activation_channel, weight_idx, 1'b1, 1'b0, 1'b0, 1'b0, 1'b0));  // Compute, Don't store, No ReLU, No Pool, Load Activation
    for(j=0; j<4; j=j+1) begin      // 4 columns
        for(m=0; m<8; m=m+1) begin      // 8 sets (layers) of Activations
            for(i=0; i<4; i=i+1) begin
...
task input_fc_task; begin
    ...
    for(m=0; m<8; m=m+1) begin
        ...
        if(m==7) instruction = pack_instruction('0, compute_op(activation_channel, weight_idx, 1'b1, 1'b1, 1'b0, 1'b0, 1'b0));  // Compute, Store, No ReLU, No Pool, Don't load Activation
        else instruction = pack_instruction('0, compute_op(activation_channel, weight_idx, 1'b1, 1'b0, 1'b0, 1'b0, 1'b0));  // Compute, Don't store, No ReLU, No Pool, Don't load Activation
    end
