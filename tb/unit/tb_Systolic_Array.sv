...
// Output Registers
reg clk, rst_n;
reg signed [7:0] DRAM_weight_data [0:8][0:7];
reg padded_input_valid [0:8];
reg signed [7:0] padded_input_data [0:8];

// Input Signals
reg sum_valid [0:7];
reg signed [31:0] sum_out [0:7];
...
reg signed [7:0] weight [0:8][0:7];
reg signed [7:0] input_in [0:8];

reg signed [31:0] expected_value [0:`PAT_NUM-1][0:7];
...
initial begin
    ...
    `endif
    `ifdef GATE
        $sdf_annotate("./Netlist/Systolic_Array_SYN.sdf", m1);
...
task reset_task; begin 
    ...
    for(i=0; i<8; i=i+1) begin
        if ((sum_valid[i] !== 1'b0) || (sum_out[i] !== 8'b0)) begin
            $display("************************************************************");
            $display("                          FAIL!                           ");
...
task generate_input_task; begin
    ...
    for(i=0; i<9; i=i+1) begin
        for(j=0; j<8; j=j+1) begin
            weight[i][j] = $random(seed);
        end
        $display("%d, %d, %d, %d, %d, %d, %d, %d",
            weight[i][0], weight[i][1], weight[i][2], weight[i][3],
            weight[i][4], weight[i][5], weight[i][6], weight[i][7]);                        
    end

    // GENERATE INPUT
    value_available = 0;
    while(value_available == 0) begin
        value_available = 1;
        for(i=0; i<9; i=i+1) begin
        ...
        for(j=0; j<8; j=j+1) begin
            check = 0;
            for(i=0; i<9; i=i+1) begin
                input_mult = input_in[i];
                weight_mult = weight[i][j];

                check = check + input_mult * weight_mult;
                // if((check > 127) || (check < -128) || 
                //    (input_mult * weight_mult > 127) || (input_mult * weight_mult < -128)) begin
                //     value_available = 0;    // repeat if value not available
                // end
            end
            expected_value[i_pat][j] = check;
            //$display("check_value_in = %f", check);
        end
    end
end endtask
...
task input_weight_task; begin
    for(i=0; i<9; i=i+1) begin
        for(j=0; j<8; j=j+1) begin
            DRAM_weight_data[i][j] = weight[i][j];
        end
    end
...
task input_task; begin
    ...
    for(i=0; i<9; i=i+1) begin
        padded_input_valid[i] = 1'b1;
        padded_input_data[i] = input_in[i];

        @(negedge clk);
        padded_input_valid[i] = 1'b0;
    end
end endtask

task wait_begin_check_task; begin
    while(sum_valid[0] !== 1) begin
