...
parameter IN_ROWS = 14;
parameter IN_COLS = 14;
parameter PAD_ROWS = 14;
parameter PAD_COLS = 14;
...
//---------------------------------------------------------------------
reg signed [7:0] image [0:PAD_ROWS-1][0:PAD_COLS-1][0:7]; // Padded image
reg [64*256-1:0] line;
reg [63:0] temp_data [0:IN_COLS-1];

reg signed [7:0] weight [0:8][0:15][0:7];    // INT8, 9 weights, 16 output channels, 8 page of weights
...
initial begin
    `ifdef RTL
        $fsdbDumpfile("CONV2.fsdb");
        $fsdbDumpvars(0,"+mda");
    `endif
    `ifdef GATE
        $sdf_annotate("./Netlist/Accelerator_SYN.sdf");
        $fsdbDumpfile("CONV2_SYN.fsdb");
        $fsdbDumpvars(0,"+mda"); 
    `endif
end
...
initial begin
    ...
    instruction = {LOAD, 3'b0, 5'b0, 1'b1, 1'b0, 1'b0, 1'b0, 1'b0};  
    ...
    @(negedge clk);
    read_image_task;
    generate_conv2_param_task;
    input_conv2_param_tile1_task;

    input_conv2_activation_task;
    for(i_pixel = 0; i_pixel < 6; i_pixel = i_pixel + 1) begin
        for(j_pixel = 0; j_pixel < 6; j_pixel = j_pixel + 1) begin
            input_conv2_task;
        end
    end
...
initial begin
    ...
    // end
    $display("************************************************************");  
    $display("                    Conv2 Layer (Tile 1)                    ");
    $display("     INPUT: 8 IFMAPS * 14 * 14 Pixels (No Padding)          ");
    $display("     OUTPUT: First 8 OFMAPS * 6 * 6 Pixels (ReLU + Pooled)  ");         
    $display("************************************************************");

    for(i_out = 0; i_out < 6; i_out = i_out + 1) begin
        for(j_out = 0; j_out < 6; j_out = j_out + 1) begin
    ...
    $display("     INPUT: 8 IFMAPS * 14 * 14 Pixels (No Padding)          ");
    $display("     OUTPUT: Second 8 OFMAPS * 6 * 6 Pixels (ReLU + Pooled) ");         
    $display("************************************************************");
...
task read_image_task; begin
    file = $fopen("CONV1_OF_Map.txt", "r");
    if (file == 0) begin
        $display("Error: Failed to open CONV1_OF_Map.txt");
        $finish;
    end
    ...
    // Load 14×14 image directly (no +1 offsets)
    for (i = 0; i < IN_ROWS; i = i + 1) begin
        if (!$feof(file)) begin
            status = $fgets(line, file);
            if (status != 0) begin
                status = $sscanf(line,
                    "%016h %016h %016h %016h %016h %016h %016h %016h %016h %016h %016h %016h %016h %016h",
...
// Conv1 Task
task generate_conv2_param_task; begin
    // GENERATE WEIGHT
    file = $fopen("conv2_weights_tb.txt", "r");
    if (file == 0) begin
    ...
    // GENERATE SCALE
    scaling_factor = 5'd10;

end endtask

task input_conv2_param_tile1_task; begin
    @(negedge clk);
    weight_idx = 0;
    instruction_valid = 1'b1;
    ...
    for(m=0; m<8; m=m+1) begin      // 8 pages of weights
...
task input_conv2_param_tile2_task; begin
    @(negedge clk);
    weight_idx = 0;
    instruction_valid = 1'b1;
    instruction = {LOAD, 3'b0, weight_idx, 1'b0, 1'b0, 1'b1, 1'b0, 1'b0};
...
task input_conv2_activation_task; begin
    @(negedge clk);
    instruction_valid = 1'b1;
...
task input_conv2_task; begin
    for(m=0; m<8; m=m+1) begin
        ...
        if(m==7) begin
            if((i_pixel == 5) && (j_pixel == 5)) begin
                instruction = {MAC_COMPUTE, activation_channel, weight_idx, 1'b1, 1'b1, 1'b1, 1'b1, 1'b0};
                    // Compute, Store, ReLU, Pool, Don't Load Activation
            end
            else begin
                instruction = {MAC_COMPUTE, activation_channel, weight_idx, 1'b1, 1'b1, 1'b1, 1'b1, 1'b1};
...
task YOU_PASS_task; begin    
    ...
    outfile = $fopen("CONV2_Tile1_OF_Map.txt", "w");
    if (outfile == 0) begin
    ...
    // Open file for writing
    outfile = $fopen("CONV2_Tile2_OF_Map.txt", "w");
    if (outfile == 0) begin
        $display("Error: Cannot open output file.");
        $finish;
    end
