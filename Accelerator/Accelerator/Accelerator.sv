...
`include "../Triangle_Buffer/Triangle_Buffer.sv"
...
reg signed [7:0] weight_data_bank [0:8][0:7][0:31];   // INT8, 9 by 8 array, 32 sets of weight
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n) begin
        ...
        for(i=0; i<9; i=i+1) begin
            for(j=0; j<8; j=j+1) begin
                weight_data_bank[i][j][weight_idx] <= SRAM_weight_data[i][j];
            end
        end
    end
    else begin
        for(i=0; i<9; i=i+1) begin
            for(j=0; j<8; j=j+1) begin
                for(m=0; m<32; m=m+1) begin
                    weight_data_bank[i][j][m] <= weight_data_bank[i][j][m]; 
                end
            end
        end
    end
...
always @(*) begin
    for(i=0; i<9; i=i+1) begin
        for(j=0; j<8; j=j+1) begin
            if(mode) begin          // Fully Connect
                weight_data[0][i][j] = weight_data_bank[i][j][weight_set[i+j] + 0];
                weight_data[1][i][j] = weight_data_bank[i][j][weight_set[i+j] + 8];
                weight_data[2][i][j] = weight_data_bank[i][j][weight_set[i+j] + 16];
                weight_data[3][i][j] = weight_data_bank[i][j][weight_set[i+j] + 24];
            end
            else begin              // Convolution
                for(m=0; m<4; m=m+1) begin
                    weight_data[m][i][j] = weight_data_bank[i][j][weight_set[i+j]];    // step count
                end
            end
        end
    end
